...
This file was 211,000 characters and well over the size limit, which meant it
was being truncated — the fences at the bottom were not reliably being read.
On **2026-09-19** every long narrative section moved, **verbatim and
complete**, into `docs/reference/`. Nothing was rewritten, summarised away or
dropped; the reasoning, the approval records, the measured numbers and the
"reported, not fixed" lists are all still there.
...
moved is why each rule exists. ⛔ **Read the reference file for a feature
before changing that feature** — the rules below are the conclusions, and
several of them are one careless reading away from their opposite.

⛔ **Do not grow this file back.** A new feature gets its ⛔ rules here, in a
few lines, and its reasoning in `docs/reference/`. If a section here passes
roughly a screen of text, it belongs in a reference file with a pointer.

| Feature | Reference |
|---|---|
| Symptom triage, emergency screening, the corpora | `docs/reference/symptom-triage.md` |
| The symptom picker and its vocabulary | `docs/reference/symptom-picker.md` |
| MedlinePlus content and retrieval | `docs/reference/medlineplus-content.md` |
| Health goals and the plan prompt | `docs/reference/health-goals.md` |
| Medication reminders and refill alerts | `docs/reference/medication-reminders.md` |
| Medication label scanning | `docs/reference/medication-label-scanning.md` |
| Appointments and provider search | `docs/reference/appointments-and-providers.md` |
| Emergency card | `docs/reference/emergency-card.md` |
| Security posture: keys, tokens, CORS, headers | `docs/reference/security-posture.md` |
| Public deployment | `docs/reference/public-deployment.md` |
| Open data-handling findings, in full | `docs/reference/open-findings.md` |
| Visual direction, web layout, home screen | `docs/reference/visual-direction.md`, `web-layout.md`, `home-screen.md` |
...
cases and `common_illness/` scores 10,000, both now at 100% agreement and 100%
safety of advice — but gold labels are **this app's own documented intent,
...
**Everything else a reviewer must be told** — the follow-up questions being
part of the instrument, `model_confidence` being recorded and never acted on,
the two-round cap, the descriptions not being encrypted at rest, and the four
"reported, not fixed" items — is in `docs/reference/symptom-triage.md`. So is
the same review requirement for `followup.py`, `dose_schedule.py`, the
emergency phrase lists, the picker vocabulary and `PLAN_SYSTEM_PROMPT`.
...
drooping". Match both word orders. ⛔ These are fenced modules: see the fences
above. Every past edit to them was individually approved by the owner in
conversation, and each approval is recorded in the reference file.

## The rules, by feature

Conclusions only. ⛔ Read the reference file before changing any of these.

### Medical content — `docs/reference/medlineplus-content.md`
...
- `search_terms.py` reduces free text to at most three keywords. Retrieval
  ...
  Its three rules exist because of measured wrong answers; re-measure rather
  than reason about changes.
...
### Symptom picker — `docs/reference/symptom-picker.md`
...
- ⛔ It runs on the device and makes **no network call**. An autocomplete is a
  GET with the user's health text in a query string, which is forbidden here.
...
- ⛔ **Every offered phrase must be screenable.**
  ...
  true of ten entries, seven already shipped.
- ⛔ **Areas are a menu, not chips** (2026-09-19). Body areas render as
  full-width rows and opening one replaces the list; drawing an area like a
  symptom chip made "head" and "eyes" read as things you could have.
...
- Guidance renders **above all other content**, routes to 911 (988 for
  self-harm), never names a condition or treatment, and is returned **even
...
- `normalize_query` splits lowercase-to-uppercase boundaries, and
  ...
  **more** sensitive. ⛔ `_VOIDED_BY_PREFIX` (food poisoning) is the only
  narrowing in the file; do not add an entry to quieten a false positive.
...
- The mirrored medication list is name and dosage only, at most 25 rows, 300
  characters each, and `clearCard()` removes it too.
...
### Medication reminders — `docs/reference/medication-reminders.md`
...
- Refill alerts: ⛔ `forecast()` **never takes a `frequency` argument** and
  ...
  `refill_date` (the user's own date) and `refill_estimate` (arithmetic) apart.
...
### Health goals — `docs/reference/health-goals.md`

⛔ **Read that file before touching this feature.** The deterministic guards
were removed at the owner's request on 2026-09-12, so `PLAN_SYSTEM_PROMPT` is
now the *only* thing constraining an authored plan. A prompt fails open and
silently. Two tests pin the removal so it cannot be quietly undone.
...
### Security posture — `docs/reference/security-posture.md`
...
### Deployment — `docs/reference/public-deployment.md`, `docs/deployment.md`
...
### UI conventions — `docs/reference/visual-direction.md`, `web-layout.md`, `home-screen.md`
...
- ⛔ **`accessibilityState` does nothing on React Native Web 0.19.13. Say the
  state in `accessibilityLabel`.** Keep `accessibilityState` for native. ⛔ A
  test asserting `accessibilityState` is not evidence about a browser — it
  passes in jsdom whether or not anything reaches the DOM. Assert the label.
  `mobile/__tests__/accessibleState.test.ts` holds this; ⛔ `EXEMPT` is not a
  snooze button.
...
break each rule these features claim to enforce and check the suite notices.
⛔ **A passing suite is evidence about the tests, not about the code** — four
"green for the wrong reason" tests were found this way. ⛔ A survivor is fixed
by fixing the test, not by deleting the mutation, and a mutation that changes
nothing also reports SURVIVED, so read the diff it implies. Neither touches
the working tree; both copy, break the copy, and delete it.

## Open data-handling findings

Full text, including the seven closed with tests:
`docs/reference/open-findings.md`.
